...
namespace TreeDS
{
    class DirectedGraph
    {
        ...
        private void FindLoop(int start)
        {
            int[] visited = new int[this._vertices];

            Stack<int> stack = new Stack<int>();
            stack.Push(start);
            visited[start] = 1;

            Queue<int> trackCycle = new Queue<int>();

            do
            {
                int vertex = stack.Pop();
                trackCycle.Enqueue(vertex);
        ...
        public void PrintStack(Queue<int> queue)
        {
            do
            {
